...
const std::array<std::string,107> keys =
...
public:
	...
	Input(std::unordered_map<int, std::unordered_map<std::string, bool>>& cmds) : get_command(cmds)
	{
	}
	...
	std::unordered_map<unsigned short,std::string> key_cmd;

	void initialise_command(int user_id, const std::string& key, const std::string& command, bool val = 0)
	{
		key_cmd[ get_key_id(key) ] = command;
		get_command[user_id][command] = val;
	}
	
	bool get(const std::string& cmd)
	{
